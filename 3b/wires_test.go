	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, want := range tt.want {
				assert.Contains(t, got, want, "result did not contain some desired intersections")
			}
			assert.Len(t, got, len(tt.want), "result contained too many intersections")
		})
	}
}
...
func TestShortestDelayCrossing(t *testing.T) {
	tests := []struct {
		name    string
		grid    Grid
		delay   int
		...
		{"basic example",
			MustToGrid([]string{"R8,U5,L5,D3", "U7,R6,D4,L4"}),
			30, false,
		}, {"example 1",
			MustToGrid([]string{
				"R75,D30,R83,U83,L12,D49,R71,U7,L72",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, actualDelay, err := ShortestDelayCrossing(tt.grid)
			if tt.wantErr {
				assert.Error(t, err)
				return
			}
			assert.Nil(t, err)
			assert.Equal(t, tt.delay, actualDelay, "delay incorrect")
		})
	}
}

...
func TestExecute(t *testing.T) {
	...
	for i, test := range []test{
		...
		t.Run(fmt.Sprintf("execute test #%d", i), func(t *testing.T) {
			...
			actual, actualErr := execute(test.program, bytes.NewBufferString(test.input), actualOut)
			if test.err {
...
func TestInput(t *testing.T) {
	...
	for i, test := range []test{
		...
		t.Run(fmt.Sprintf("Input Test #%d", i), func(t *testing.T) {
			actual, actualErr := execute(test.program, bytes.NewBufferString(test.input), nil)
			if test.err {
...
func TestOutput(t *testing.T) {
	...
	for i, test := range []test{
		...
		t.Run(fmt.Sprintf("output test #%d", i), func(t *testing.T) {
			output := &bytes.Buffer{}
			actual, actualErr := execute(test.program, nil, output)
			if test.err {
				assert.Error(t, actualErr)
				return
			}
...
func TestReadOp(t *testing.T) {
	...
	for i, test := range []test{
		...
		t.Run(fmt.Sprintf("read op test #%d", i), func(t *testing.T) {
			op, err := ReadOp(test.program, 0, len(test.args))
			if test.err {
				assert.Error(t, err)
				return
			}
